...
@app.route('/super_register', methods=('GET', 'POST'))
def super_register():
    ...
    return render_template('super_register.html', form=form)
...
def login():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if user.super:
            return render_template('super_loggedin.html')
...
def get_book():
    try:
        if session['is_login']:
            ...
            curuser = User.query.filter(User.id == user_id).first()
            if curuser.super:
...
@app.route('/increase_book', methods=('POST', 'GET'))
def increase_book():
    try:
        if session['is_login']:
            user_id = session['user_id']
...
@app.route('/decrease_book', methods=('POST', 'GET'))
def decrease_book():
    try:
        if session['is_login']:
            user_id = session['user_id']
...
def rent_book():
    try:
        if session['is_login']:
            ...
            error = ''
            rental_already = Rental.query.filter(
            ...
            if book.stock <= 0:
                ...
                new_rental = Rental(user_id=user_id, book_id=book_id)
                book.stock -= 1
    ...
    try:
        if session['is_login']:
            ...
            if request.method == 'POST':
                ...
                rental = Rental.query.filter(Rental.id == rental_id).first()
    ...
    try:
        if session['is_login']:
            ...
            if curuser.super:
                ...
                return render_template('super_log.html', rentals=rentals)
    except KeyError:
        return redirect(url_for('login'))
...
def get_book_detail(book_id):
    try:
        if session['is_login']:
            ...
            saying = wisesaying[random.randint(0, len(wisesaying)-1)]
            user_id = session['user_id']
            curuser = User.query.filter(User.id == user_id).first()
            if curuser.super:
                return render_template(
                        'super_book_detail.html', book=book,
                        comments=comments, saying=saying
                        )
            return render_template(
